...
function demoPrediction(input, originalName, quality) {
  ...
  let classCode = detectHint(originalName);
  if (!classCode) {
    if (quality.score < 48) classCode = 'unknown';
    else if (quality.greenDominance > 1.32 && quality.entropy < 6.7) classCode = 'healthy';
    else classCode = classes[hash[0] % classes.length];
  }
...
export async function analyzeImage(filePath, originalName = '') {
  ...
  const hash = crypto.createHash('sha256').update(input).digest();
  const lesionRatio = category === 'health' ? 0 : category === 'unknown' ? null : Number(clamp(0.018 + (1 - confidence) * 0.16 + (hash[2] / 255) * 0.035, 0.01, 0.26).toFixed(4));
  ...
  const needsReview = category === 'unknown' || confidence < Number(process.env.REVIEW_THRESHOLD || 0.72) || quality.score < 55;
  ...
  return {
    ...
    explanation: category === 'unknown'
      ...
      : category === 'health'
        ...
        : shouldUseModel
          ? '结果来自已训练的图像分类模型；严重度为图像质量、置信度和疑似异常占比的辅助估计。'
          : '当前为演示分析器输出；接入模型后可切换为真实分类推理。',
    disclaimer: shouldUseModel
      ? '该模型为分类基线，可用于项目演示和初筛；田间生产应用前仍需外部茶园盲测、专家复核和持续数据回流。'
