...
export const supportedClasses = ['healthy', 'algal_leaf', 'anthracnose', 'bird_eye_spot', 'brown_blight', 'gray_blight', 'red_leaf_spot', 'white_spot', 'tea_white_scab', 'tea_blister_blight', 'tea_blister_blight_perforation', 'leaf_beetle', 'apolygus_lucorum', 'unknown'];
...
function modelStatus() {
  return {
    version: 'tea-disease-pest-mobilenetv3-open-set',
    mode: process.env.ANALYZER_MODE || 'demo',
    supportedClasses,
    modelFile: 'models/tea_disease_pest.onnx',
    metricsFile: 'models/metrics.json',
    notice: '真实分类模型用于茶树健康/病虫害初筛，并在低置信度或非茶叶图像时返回 unknown；不输出病斑框、虫体框或严重度。'
  };
}
